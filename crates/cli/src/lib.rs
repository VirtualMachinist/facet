...
/// Exit code used when a completed response misses a `--expect` assertion.
pub const EXPECTATION_EXIT_CODE: u8 = 9;
...
pub const fn help() -> &'static str {
    concat!(
        ...
        "  request list <path>                 List requests\n",
        "  request get <path> <selector>       Inspect a request\n",
        "  request variables <path> <selector> Discover variables referenced by a request\n",
        "  request run <path> <selector>       Execute a request\n",
        "  request set <path> <selector>       Set and persist HTTP or GraphQL request fields\n",
        ...
        "      --graphql-extensions <json> Set GraphQL extensions as a JSON object or null\n",
